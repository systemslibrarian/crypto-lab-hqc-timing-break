...
Set the secret message size, the inner-code redundancy, the cache noise, and how many Flush+Reload probes to average per position, then run the attack. The chart shows the per-position cache hit-rate — on the optimized binary, bars split above and below the 50% line by secret bit; bars are green when read correctly, red when noise flipped them. A legend above the chart states the dual encoding explicitly (height = hit-rate = secret bit, color = whether noise flipped the read), and **hovering or focusing any bar** lights up the matching branch in a mini compiled-code panel and narrates the causal chain for that one position — *secret bit = 1 → branch touches line a → probe HIT* — so each bar is visibly one firing of the leaking select rather than an abstract statistic. Beside the Flush+Reload steps, a **"Why a cache hit leaks a secret"** primer with a fast-vs-slow timing strip builds the shared-line / flush / reload mental model a newcomer needs. The recovery panel compares **Soft-ISD** against plain **hard-decision** majority and reports both accuracies and the total probe count; an expandable **"Why Soft-ISD beats majority vote"** trace picks one message bit and shows every carrying position's hit-rate, reliability weight, and signed LLR vote, then sets the raw head-count tally beside the weighted LLR sum — surfacing the case (most visible under the *Noisy co-tenant* preset) where a cluster of ambiguous positions wins the majority but loses the weighted vote. Toggle **Constant-time binary** and re-run: every select now touches the probed line, every bar pins near 100%, and recovery drops to chance. Below the lab, a "Same Source, Two Binaries" panel shows the exact mask-select C source and the branchy code the optimizer emits, followed by the timeline of HQC's four documented timing/cache leaks and a do/don't guide to keeping the binary honest.
