 ...
 *    files the node under `incomplete`. This lab is full of them: every timing
 *    bar, every `.vs-chip` verdict badge and the hero all paint
 *    `linear-gradient` fills, so the text on them composites onto something axe
 *    will not measure.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. `.cl-hero-sub` here carries
 *    `opacity: .85`, and several `.is-muted`/disabled treatments fade whole
 *    subtrees. Note this lab collapses its animations to 0.01ms under reduced
 *    motion, which preserves end states.
 *
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. The two
 *    `.code-block` panels hold the C source and its `-O3` compilation with
 *    `white-space: pre`, and `.soft-vote-scroll` wraps a four-column LLR table;
 *    all three are `overflow-x: auto`, so at 380px much of their content is
 *    scrolled out of the wrapper's client box. Content scrolled past that box is
 ...
 *    viewport where the very same content is visible and measured for real.
 *
 ...
 *    draws its timing chart out of plain `<button class="bar">` elements rather
 *    than SVG, and its only SVG is the header's `aria-hidden` hamburger icon —
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     * accents into surfaces with `color-mix(in oklab, ...)` in 46 places — every
     * bar fill, chip and panel tint — and Chromium reports those
     ...
     *
     * This page renders one `<button class="bar">` per codeword position (12
     * message bits x 7 redundancy = 84 at the sliders' maxima), a recovered and
     * an actual bit cell per message bit, and a four-column LLR table row per
     * position in the traced bit — every one of them walks the same handful of
     * ancestors. Without
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This page's disclosures are `.bit-details` (bit-by-bit
      // recovery) and `.soft-vote` (the per-position LLR table), and the gate
      // scans both of their open states explicitly.
      ...
      // `.skip-link` ("Skip to interactive lab") each park off-screen and slide
     ...
     * it, the seed-lock button's `aria-hidden` padlock (🔓/🔒) and the
     * `.fs-strip` cache-timing diagram (entirely `aria-hidden`) are measured as
     * if each glyph took its element's declared `color` — but an emoji paints in
     * its own multicolour, so that ratio describes nothing on screen.
     */
